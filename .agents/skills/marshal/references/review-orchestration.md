...
3. **对抗式验证二段(抬高误报地板)**:对 quorum 后存活的每条发现(`confirmed` + `escalate`),先取互异 refute 视角:
   ...
   （⑧ review trace，默认启用）审前 `review-run-open --change-ref <sha> --repo <repo> --mode regular|deep --host codex --model <模型id>` 拿 `run_id`;上面 `review-verify` 追加 `--run-id <id> --findings-json '[{key,title,claim,location,lens},…]'` 即把每条发现的裁决链落库;用户终审后 `finding-verdict --finding-id <id> --verdict accepted|rejected|modified` 补录金标注。不带 `--run-id` 行为不变。
